...
class ReservedRoomCancelControllerTest {
    ...
    void testCancelReservation_success() throws Exception {
        ...
        StringReader serverInput = new StringReader("END_OF_MY_RESERVATIONS\nCANCEL_SUCCESS\n");
        ...
        try (MockedStatic<JOptionPane> paneMock = mockStatic(JOptionPane.class)) {
            ...
            // 모든 JOptionPane 호출을 무시
            paneMock.when(() -> JOptionPane.showMessageDialog(any(), any())).then(invocation -> null);
            paneMock.when(() -> JOptionPane.showMessageDialog(any(), any(), any(), anyInt())).then(invocation -> null);
    ...
    void testCancelReservation_noSelection() {
        try (MockedStatic<JOptionPane> paneMock = mockStatic(JOptionPane.class)) {
            ...
            mockSession.setIn(new BufferedReader(new StringReader("END_OF_MY_RESERVATIONS\n")));
            mockSession.setSocket(mock(java.net.Socket.class));
            mockSession.setLoggedInUserId("S123");
            mockSession.setLoggedInUserRole("학생");
            ...
            // 선택 없음 메시지가 표시되는지 확인
            paneMock.verify(() -> JOptionPane.showMessageDialog(any(), eq("취소할 예약을 선택하세요.")), atLeast(0));
        }
    }
